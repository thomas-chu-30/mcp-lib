...
const server = new McpServer({ name: "outline-reader", version: "1.0.0" }, { capabilities: { tools: {} } });
// 1. 註冊工具：讓 AI 知道可以「讀取文件」與「搜尋文件」
server.registerTool("get_outline_document", {
    ...
    const result = await outlineFetch("documents.info", { id });
    if (!result.ok) {
        return {
...
server.registerTool("search_outline", {
    ...
    const result = await outlineFetch("documents.search", { query });
    if (!result.ok) {
        return {
            content: [{ type: "text", text: `Outline API 錯誤：${result.error}` }],
            isError: true
    ...
    const lines = list.map((hit) => `[${hit.document?.title ?? "無標題"}] ID: ${hit.document?.id ?? ""}`);
    return {
